...
void PlayerSelector::Map::draw()
{
    ...
    for (const std::string &line : _charMap) {
        ...
        for (const char &c : line) {
            ...
            if (c == 'M')
                Raylib::Drawing::drawCubeTexture(_textures[current][BOX], {x * _COEF - (float)2.4, y * _COEF - (float)0.5, 0}, _COEF, _COEF, _COEF, Raylib::Color::White().getCStruct());
            Raylib::Drawing::drawCubeTexture(_textures[current][FLOOR], {x * _COEF - (float)2.4, y * _COEF - (float)0.5, -_COEF}, _COEF, _COEF, _COEF, Raylib::Color::Gray().getCStruct());
            x++;
        }
        y++;
    }
