...
void Game::Bomberman::initEntities() //TODO: pushback player //TODO: floor under !
{
    ...
    for (const std::string &line : _map) {
        ...
        for (const char &c : line) {
            ...
            {
                if (c == 'H')
                    _entities.push_back(std::shared_ptr<IEntity>(new Game::Human(_players[dic_index].name, {x, y, 0}, _players[dic_index].obj, _players[dic_index].texture, _players[dic_index].animations, _players[dic_index].color)));
                else if (c == 'A')
                    _entities.push_back(std::shared_ptr<IEntity>(new Game::AI(_players[dic_index].name, {x, y, 0}, _players[dic_index].obj, _players[dic_index].texture, _players[dic_index].animations, _players[dic_index].color)));
                dic_index++;
            }
            else if (c == 'W' || c == 'E')
